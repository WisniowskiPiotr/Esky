...
do           
  from=$(echo $from | sed "s/ - .*//" )
	...
	do           
		...
		while read date_from           
		do           
			date_from=$(echo $date_from | sed "s/ - .*$//" )
			while read date_dest           
			do           
				date_dest=$(echo $date_dest | sed "s/^.* - //" )

				tmp_file=$base_path/site
				...
				echo "Found on Esky $ot $from to $dest on $date_from - $date_dest price $price"
